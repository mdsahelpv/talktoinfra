# TalkAI Platform - Development Tasklist

## Phase 0: Infrastructure Onboarding & Mock Data Removal (CRITICAL - MUST BE FIRST)

### 0.1 Mock Data Removal - Production Readiness

**GOAL: Remove ALL mock/fake data from the application. ZERO mock data in production.**

- [ ] **Remove mock Kubernetes client** (`services/ingestion-service/k8s_client.py`)
  - [ ] Delete `_get_mock_resources()` method entirely
  - [ ] Delete `get_mock_namespaces()` method entirely
  - [ ] Implement real `config.load_kube_config()` or `config.load_incluster_config()`
  - [ ] Add proper error handling for connection failures
  - [ ] Add authentication (token, cert, or OIDC)
  - [ ] Test with real cluster - no fallback to mock data

- [ ] **Remove mock data from Agent Service**
  - [ ] Audit all tool implementations for mock data
  - [ ] Replace mock K8s tool responses with real API calls
  - [ ] Remove any hardcoded test responses
  - [ ] Ensure tools fail gracefully with proper error messages

- [ ] **Remove mock data from API Gateway**
  - [ ] Remove hardcoded "local" cluster references
  - [ ] Remove mock discovery responses
  - [ ] Remove mock resource endpoints
  - [ ] All endpoints must connect to real services only

- [ ] **Remove mock data from all tests**
  - [ ] Update test fixtures to use testcontainers or real (disposable) resources
  - [ ] Mock external APIs only, never internal logic
  - [ ] Add integration tests against real infrastructure
  - [ ] Create test-specific kubeconfig for CI/CD

- [ ] **Add "NO_MOCK_DATA" enforcement**
  - [ ] Create pre-commit hook to detect mock data patterns
  - [ ] Add CI/CD check that fails if mock data detected in production code
  - [ ] Document "No Mock Data" policy in README
  - [ ] Create ADR (Architecture Decision Record) explaining the policy

### 0.2 Infrastructure Onboarding Service (NEW - Port 8011)

**This is the FOUNDATION - the app cannot work without real infrastructure connections.**

- [ ] **Create Onboarding Service** (Port 8011)
  - [ ] Define database schema for clusters, cloud accounts, and credentials
  - [ ] Implement secure credential storage (Vault/AWS Secrets Manager/Azure Key Vault)
  - [ ] Create API endpoints for registration
  - [ ] Add connection validation and health checks
  - [ ] Implement credential rotation automation

- [ ] **Kubernetes Cluster Onboarding**
  - [ ] API endpoint: `POST /api/v1/clusters/register`
    - [ ] Accept kubeconfig (base64 encoded or file upload)
    - [ ] Accept service account token + API endpoint
    - [ ] Accept OIDC/OAuth configuration
    - [ ] Support multiple auth methods
  - [ ] Validate cluster connection immediately
  - [ ] Test permissions (can we list pods? get nodes?)
  - [ ] Select namespaces to monitor (allowlist/blocklist)
  - [ ] Set cluster labels/tags (env: production, region: us-east-1)
  - [ ] Encrypt and store credentials securely
  - [ ] Create cluster health monitoring job

- [ ] **Cloud Provider Account Onboarding**
  - [ ] **AWS Account Registration**
    - [ ] API endpoint: `POST /api/v1/cloud/aws/register`
    - [ ] Support IAM Role (IRSA for EKS, instance profile for EC2)
    - [ ] Support access key + secret key (with rotation)
    - [ ] Support AWS SSO/OIDC
    - [ ] Test STS GetCallerIdentity to validate credentials
    - [ ] Select regions to scan
    - [ ] Choose resource types to discover (EC2, RDS, S3, etc.)
  - [ ] **Azure Subscription Registration**
    - [ ] API endpoint: `POST /api/v1/cloud/azure/register`
    - [ ] Support Service Principal (client ID/secret)
    - [ ] Support Managed Identity
    - [ ] Support Azure CLI authentication (dev only)
  - [ ] **GCP Project Registration**
    - [ ] API endpoint: `POST /api/v1/cloud/gcp/register`
    - [ ] Support Service Account JSON key
    - [ ] Support Workload Identity (for GKE)
    - [ ] Support Application Default Credentials (dev only)

- [ ] **Credential Security & Management**
  - [ ] Integrate with HashiCorp Vault
    - [ ] Use Vault KV v2 for secret storage
    - [ ] Implement dynamic secrets where possible
    - [ ] Enable automatic credential rotation
  - [ ] Implement credential encryption at rest
    - [ ] AES-256 encryption for database storage
    - [ ] Separate encryption key per tenant (for multi-tenancy)
  - [ ] Add credential health monitoring
    - [ ] Periodic "can we still connect?" checks
    - [ ] Alert on expired credentials
    - [ ] Alert on permission changes (access denied errors)
  - [ ] Implement credential versioning
    - [ ] Keep history of credential changes
    - [ ] Support quick rollback to previous credentials
    - [ ] Audit log all credential access

- [ ] **Connection Testing & Validation**
  - [ ] `POST /api/v1/clusters/{id}/test-connection` endpoint
  - [ ] `POST /api/v1/cloud/{provider}/{id}/test-connection` endpoint
  - [ ] Return detailed error messages (connection refused, auth failed, etc.)
  - [ ] Test with minimal permissions first, escalate as needed
  - [ ] Create connection diagnostic tool (CLI or API)

### 0.3 Frontend Onboarding UI

- [ ] **Build "Connect Infrastructure" Wizard**
  - [ ] Step 1: Select type (Kubernetes / AWS / Azure / GCP)
  - [ ] Step 2: Provide credentials (with help text and examples)
  - [ ] Step 3: Test connection (with loading spinner)
  - [ ] Step 4: Select resources/namespaces to monitor
  - [ ] Step 5: Review and confirm
  - [ ] Success page with "Start Exploring" button

- [ ] **Connection Management Dashboard**
  - [ ] List all connected clusters and cloud accounts
  - [ ] Show connection status (healthy/unhealthy/error)
  - [ ] Last sync time for each connection
  - [ ] Edit/update credentials
  - [ ] Delete/disconnect button with confirmation
  - [ ] Test connection button (re-validate)
  - [ ] View connection details (namespaces, regions, resource counts)

- [ ] **Onboarding Flow for First-Time Users**
  - [ ] Welcome screen: "Connect your first cluster"
  - [ ] Guided tutorial for first connection
  - [ ] "Quick Connect" with default kubeconfig location
  - [ ] Sample data option for demo mode (clearly labeled as DEMO)
  - [ ] Skip option (but show warning that app won't work without connections)

### 0.4 Update Existing Services to Use Onboarding

- [ ] **Update Ingestion Service**
  - [ ] Fetch cluster list from Onboarding Service instead of hardcoded
  - [ ] Get credentials from Vault (via Onboarding Service)
  - [ ] Loop through all registered clusters
  - [ ] Handle multiple clusters in parallel
  - [ ] Tag resources with cluster ID

- [ ] **Update Agent Service**
  - [ ] Query Onboarding Service for available clusters
  - [ ] Let user select which cluster to operate on (or "all clusters")
  - [ ] Route K8s API calls to correct cluster context
  - [ ] Add cluster-aware tool routing

- [ ] **Update Action Engine**
  - [ ] Get credentials for target cluster from Onboarding Service
  - [ ] Validate user has permission for target cluster (check with Policy Engine)
  - [ ] Execute actions on correct cluster
  - [ ] Support cross-cluster operations (e.g., "restart all pods in all clusters")

- [ ] **Update Discovery Service**
  - [ ] Get network ranges from cloud provider APIs (not just manual input)
  - [ ] Correlate discovered hosts with K8s nodes
  - [ ] Import discovered hosts into managed hosts with proper metadata

### 0.5 Discovery → Onboarding Integration (BRIDGE)
...
- [ ] **Layer 1: Structured Storage (PostgreSQL)**
  - [ ] **Discovery Data Tables** (Already exist, enhance them):
    - [ ] `discovered_hosts` - IP, hostname, ports, banners
    - [ ] `discovered_ports` - Port, service, version, status
    - [ ] `managed_hosts` - Promoted from discovered
    - [ ] Add `scan_metadata` JSON field to store full nmap/masscan output
    - [ ] Add `service_banners` TEXT field for grab banners (HTTP headers, SSH version, etc.)
    - [ ] Add `ssl_certificate` JSON for cert info (expiry, issuer)
    - [ ] Add `last_seen_at` timestamp for tracking
  - [ ] **K8s Resource Tables** (NEW):
    - [ ] `k8s_pods` - name, namespace, status, node, images, labels
    - [ ] `k8s_deployments` - name, namespace, replicas, strategy
    - [ ] `k8s_services` - name, type, cluster_ip, ports, selectors
    - [ ] `k8s_nodes` - name, status, capacity, labels, taints
    - [ ] `k8s_events` - reason, message, timestamp, involved_object
    - [ ] `k8s_logs` - pod_name, container, timestamp, log_line, level
  - [ ] **Cloud Resource Tables** (NEW):
    - [ ] `aws_instances` - instance_id, type, state, region, tags
    - [ ] `aws_rds` - db_id, engine, version, status, endpoint
    - [ ] `azure_vms`, `gcp_instances` (similar)
  - [ ] **Relationships & Graph** (NEW):
    - [ ] `resource_relationships` - from_resource, to_resource, relation_type
    - [ ] Types: "hosts_pod", "service_targets_deployment", "node_runs_pod"
    - [ ] Use for topology, impact analysis

- [ ] **Layer 2: Searchable Document Store (Elasticsearch/OpenSearch)**
  - [ ] **Why needed:** Full-text search on logs, error messages, unstructured data
  - [ ] **Indexes:**
    - [ ] `infrastructure-logs` - Pod logs, system logs
    - [ ] `infrastructure-events` - K8s events, scan events
    - [ ] `infrastructure-configs` - ConfigMaps, Secrets (redacted values)
    - [ ] `infrastructure-docs` - Confluence, READMEs
  - [ ] **Features:**
    - [ ] Log aggregation from all clusters
    - [ ] Full-text search: "Find all errors about 'connection refused'"
    - [ ] Log pattern detection
    - [ ] Time-series aggregation

- [ ] **Layer 3: Vector Embeddings (Qdrant)**
  - [ ] **Purpose:** Semantic search for AI RAG
  - [ ] **Collections:**
    - [ ] `infrastructure-resources` - Embeddings of all infrastructure (main)
    - [ ] `infrastructure-logs` - Embeddings of log patterns
    - [ ] `infrastructure-docs` - Documentation embeddings
    - [ ] `infrastructure-runbooks` - Troubleshooting guides
...
- [ ] **Automated Indexing Pipeline**
  - [ ] **Discovery Service → RAG:**
    - [ ] Trigger: Scan job completes
    - [ ] Action: Transform all discovered hosts → text → embeddings → Qdrant
    - [ ] Batch size: 100 hosts at a time
    - [ ] Update frequency: Real-time (within 30 seconds of scan completion)
  - [ ] **K8s Ingestion → RAG:**
    - [ ] Trigger: Resource change detected via Watch API
    - [ ] Action: 
      - [ ] Pod created → Add to Qdrant
      - [ ] Pod deleted → Remove from Qdrant
      - [ ] Pod updated → Update embedding
    - [ ] Frequency: Event-driven (no polling)
  - [ ] **Incremental Updates:**
    - [ ] Don't re-index everything every time
    - [ ] Track `last_indexed_at` timestamps
    - [ ] Only index changed resources
    - [ ] Handle deletions properly (remove from Qdrant)
...
- [ ] **Event-Driven Updates (NATS)**
  - [ ] **Discovery Service publishes:**
    - [ ] `discovery.scan.completed` → Ingestion Service indexes results
    - [ ] `discovery.host.updated` → Update single host in Qdrant
  - [ ] **K8s Watcher publishes:**
    - [ ] `k8s.pod.created` → Add to Qdrant
    - [ ] `k8s.pod.deleted` → Remove from Qdrant
    - [ ] `k8s.pod.modified` → Update embedding
  - [ ] **Ingestion Service subscribes:**
    - [ ] Listen to all events
    - [ ] Update Qdrant within 5 seconds
    - [ ] Track lag metrics
